...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [1] = LAYOUT_65_ansi_blocker(
    ...
    _______, RGB_TOG, _______, _______, _______, MD_BOOT, NK_TOGG, DBG_TOG, _______, LAG_SWP, CHNG_MD, _______,          KC_VOLU, KC_BRID, \
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        case KC_S:
            if (record->event.pressed && MODS_RALT) {
              send_string_with_delay(cmds[G_STATUS - G_INIT], 1);
            }
            return false;
        case CHNG_MD:
