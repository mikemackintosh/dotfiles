...
if [[ -z $COMPUTER_NAME ]]; then
  echo "Please provide a COMPUTER_NAME and try again."
  exit 1
fi
...
if [[ $hostname != $COMPUTER_NAME ]]; then
  log "Setting computer hostname to: ${COMPUTER_NAME}"
...
# NOTE: This needs to be enabled for terminal-notifier to work correctly
...
# Top right screen corner → Application Window
...
# Disable automatic loading of remote assets
...
# Prevent disconnect on screenlock
...
# Disable remote login and sharing
REMOTE=$(sudo systemsetup -f -getremotelogin | grep -i off > /dev/null; echo $?)
if [[ $REMOTE != 0 ]]; then
  sudo systemsetup -f -setremotelogin off
fi


# Prevent printers from being shared
sudo cupsctl --no-share-printers
#sudo launchctl unload -w /System/Library/LaunchDaemons/com.apple.screensharing.plist
#sudo launchctl unload -w /System/Library/LaunchDaemons/com.apple.AppleFileServer.plist && launchctl unload -w /System/Library/LaunchDaemons/com.apple.smbd.plist
